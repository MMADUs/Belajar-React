...
export default function Order () {
    ...
    const getStatistic = async () => {
        try {
            const response = await axios.get('http://localhost:3000/order/order/statistic')
            console.log(response)
    ...
    const updateStatus = async (id) => {
        try {
            const response = await axios.put(`http://localhost:3000/order/status/${id}`, {
                newStatus: 'finished'
            }, {
        ...
        <div className="flex h-screen">
            ...
            <div className="w-full bg-neutral-100">
                ...
                <div className="flex mt-10 justify-between">
                            <div className="border bg-white shadow-lg w-[350px] border-zinc-700 p-4 rounded-lg">
                                <div className="flex justify-between">
                                    <h1 className="font-semibold">Unpaid Orders</h1>
                                    <div className="w-3 h-3 bg-red-500 rounded-full" />
                                </div>
                                <p className="text-sm text-slate-400">Total of the unpaid Orders</p>
                                <p className="font-bold text-2xl">{statistic.UnpaidCount}</p>
                            </div>
                            <div className="border bg-white shadow-lg w-[350px] border-zinc-700 p-4 rounded-lg">
                                <div className="flex justify-between">
                                    <h1 className="font-semibold">Pending Orders</h1>
                                    <div className="w-3 h-3 bg-orange-500 rounded-full" />
                                </div>
                                <p className="text-sm text-slate-400">Total of the Pending Orders</p>
                                <p className="font-bold text-2xl">{statistic.PendingCount}</p>
                            </div>
                            <div className="border bg-white shadow-lg w-[350px] border-zinc-700 p-4 rounded-lg">
                                <div className="flex justify-between">
                                    <h1 className="font-semibold">On Going Orders</h1>
                                    <div className="w-3 h-3 bg-green-500 rounded-full" />
                                </div>
                                <p className="text-sm text-slate-400">Total of the on going Orders</p>
                                <p className="font-bold text-2xl">{statistic.totalOrder}</p>
                            </div>
                        </div>
                <div className="w-[1200px] mt-10 mb-10 px-5 pb-5 bg-white shadow-lg border border-zinc-700 rounded-lg">
                    <h1 className="font-bold mt-5 text-2xl tracking-wide">On Going Pesanan</h1>
                    <p className="text-sm mb-5 mt-1 text-slate-400">Managment for list of data pesanan</p>
                        ...
                        <div className="flex">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                    ...
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem className="focus:bg-zinc-200" onClick={() => { setStatus(""); }}>All</DropdownMenuItem>
                                    <DropdownMenuItem className="focus:bg-zinc-200" onClick={() => { setStatus("pending"); }}>Pending</DropdownMenuItem>
                                ...
                                orders.map((order) => (
                                    <tr key={order._id} className="border-t border-gray-300">
                                        ...
                                        <td className="py-3 px-4">
                                            <div className={`text-sm text-white w-fit py-1 px-2 rounded-md font-semibold ${order.status_pesanan === 'unpaid' ? 'bg-red-500' : order.status_pesanan === 'pending' ? 'bg-orange-500' : ''}`}>
                                                        ...
                                                        <SheetHeader>
                                                            ...
                                                            <div className="flex flex-col h-screen">
                                                                ...
                                                                {order.status_pesanan == 'pending' ? (
                                                                    <AlertDialog>
                                                                        <AlertDialogTrigger className="sticky mt-auto bottom-4 text-center mx-6 rounded-lg bg-green-500 hover:bg-green-600 duration-200 text-white font-semibold p-4 shadow">Selesaikan Pesanan</AlertDialogTrigger>
                                                                        <AlertDialogContent>
                                                                            <AlertDialogHeader>
                                                                                <AlertDialogTitle>Menyelesaikan pesanan?</AlertDialogTitle>
                                                                                <AlertDialogDescription>
                                                                                    Apakah anda yaking ingin menyelesaikan pesanan ini? pesanan yang sudah selesai tidak dapat di proses kembali.
                                                                                </AlertDialogDescription>
                                                                            </AlertDialogHeader>
                                                                            <AlertDialogFooter>
                                                                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                                                <AlertDialogAction onClick={() => updateStatus(order._id)}>Selesai</AlertDialogAction>
                                                                            </AlertDialogFooter>
                                                                        </AlertDialogContent>
                                                                    </AlertDialog>
                                                                ) : (
                                                                    <AlertDialog>
                                                                        <AlertDialogTrigger className="sticky mt-auto bottom-4 text-center mx-6 rounded-lg bg-red-500 hover:bg-red-600 duration-200 text-white font-semibold p-4 shadow">Pesanan belum di Bayar!</AlertDialogTrigger>
                                                                        <AlertDialogContent>
                                                                            <AlertDialogHeader>
                                                                                <AlertDialogTitle>Bayar pesanan!</AlertDialogTitle>
                                                                                <AlertDialogDescription>
                                                                                    Di mohon untuk segera melakukan pesanan sebelum pesanan bisa di proses!
                                                                                </AlertDialogDescription>
                                                                            </AlertDialogHeader>
                                                                            <AlertDialogFooter>
